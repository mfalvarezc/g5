...
public class PaymentServiceTest {
    ...
    private final static String PAYMENT_SERVICE_LOCAL = "java:global/classes/PaymentService!com.g5.services.PaymentServiceLocal";
    ...
    private PaymentServiceLocal getPaymentService() throws NamingException {
        return (PaymentServiceLocal) context.lookup(PAYMENT_SERVICE_LOCAL);
    }
    ...
    public void testRollbackPayment() throws Exception {
        ...
        BigDecimal newReceiverBalance = receiverAccount.getBalance().subtract(payment.getValue());
        BigDecimal newSenderBalance = senderAccount.getBalance().add(payment.getValue());

        PaymentServiceLocal service = getPaymentService();

        long minTime = System.currentTimeMillis() / 1000 * 1000;

        service.rollbackPayment(payment.getId());
        ...
        assertEquals("Rollback - " + payment.getDescription(), transaction.getDescription());
    ...
    public void testRequestPayment() throws Exception {
        ...
        List<PaymentEntity> payments = entityManager.createNamedQuery("Payment.findAll", PaymentEntity.class).getResultList();
    ...
    public void testApprovePayment() throws Exception {
        ...
        System.out.println("Current receiver balance: " + receiverAccount.getBalance());
        System.out.println("Current sender balance: " + senderAccount.getBalance());

        BigDecimal newReceiverBalance = receiverAccount.getBalance().add(payment.getValue());
        BigDecimal newSenderBalance = senderAccount.getBalance().subtract(payment.getValue());
        ...
        service.approvePayment(payment.getId(), senderAccount.getId(), payment.getValue());
        ...
        Transaction senderTransaction = transactions.get(transactions.size() - 2);

        assertEquals(senderAccount.getId(), senderTransaction.getAccount().getId());
        assertTrue(senderTransaction.getDate().getTime() >= minTime);
        assertTrue(senderTransaction.getDate().getTime() <= maxTime);
        assertEquals(payment.getDescription(), senderTransaction.getDescription());
        assertEquals(payment.getValue().negate(), senderTransaction.getValue());

        Transaction receiverTransaction = transactions.get(transactions.size() - 1);

        assertEquals(receiverAccount.getId(), receiverTransaction.getAccount().getId());
        assertTrue(receiverTransaction.getDate().getTime() >= minTime);
        assertTrue(receiverTransaction.getDate().getTime() <= maxTime);
        assertEquals(payment.getDescription(), receiverTransaction.getDescription());
        assertEquals(payment.getValue(), receiverTransaction.getValue());

        entityManager.refresh(payment);

        assertEquals(receiverAccount.getId(), payment.getReceiverAccount().getId());
        assertEquals(receiverTransaction.getId(), payment.getReceiverTransaction().getId());
        assertEquals(senderAccount.getId(), payment.getSenderAccount().getId());
        assertEquals(senderTransaction.getId(), payment.getSenderTransaction().getId());

        entityManager.refresh(receiverAccount);

        assertEquals(0, newReceiverBalance.compareTo(receiverAccount.getBalance()));
